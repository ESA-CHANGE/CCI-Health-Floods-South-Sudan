...
Script to perform an Exploratory Dat Analysis (EDA) on the annual flood maps.

This script outputs several images and metrics to understand data quality, temporal
trends, and spatial patterns in the flood maps. It is designed to be run after the
hazard maps have been generated and saved as 4-band GeoTIFF files.
...
class FloodMapEDA:
    """
    Class for exploratory data analysis of annual flood maps.
    """
    ...
    def __init__(self, file_paths=None):
        ...
        Args:
            file_paths: Dict {year: file_path}
        """
        self.file_paths = file_paths
        self.data = {}  # Will store data by year
        self.metadata = {}
        self.years = []
        ...
        for year in self.years:
            ...
            for band in self.bands:
                ...
                if band == 'flood_frequency':
                    print(f"\nDEBUG {year} - {band}:")
                    print(f"  Total valid pixels: {len(data)}")
                    print(f"  Pixels > 0: {np.sum(data > 0)}")
                    print(f"  Total sum: {np.sum(data)}")
                    print(f"  Mean (float64): {np.mean(data)}")
                    print(f"  Max: {np.max(data)}")
                    print(f"  Original type: {self.data[year][band].dtype}")
    ...
    def spatial_consistency_analysis(self):
        """Analyzes spatial consistency between years."""
    ...
    def data_quality_assessment(self):
        """Evaluates the quality of the data."""
        quality_report = {}
        ...
        for year in self.years:
            ...
            try:
                ...
                # If conversion fails, use default value and warn
                print(f"Warning: Invalid OBSERVED_DAYS for {year}: '{obs_days_raw}'. Using 365.")
                obs_days = 365
            ...
            if year == 2012:
                ...
                # Specific case: freq=0 but consecutive>0
            ...
            # Inconsistencies: max_consecutive > duration
            inconsistent_consec = np.sum(max_consec > (dur + 1e-5))
            
            # Inconsistencies: freq=0 but dur>0 or consec>0
    ...
    def visualize_basic_maps(self):
        """Basic visualization of the maps - a large image with all years."""
        ...
        # Ensure axes is 2D even if there's only one year
        if n_years == 1:
    ...
    def visualize_statistics(self):
        ...
        for idx, band in enumerate(self.bands):
            ...
            ax.set_title(f'{band}\n(Just Flooded Pixels >0)', fontsize=11, fontweight='bold')
            ax.set_xlabel('Year')
            ax.set_ylabel('Value')
            ax.grid(True, alpha=0.3, axis='y')
        
        plt.suptitle('Distribution by Band - Only Flooded Pixels (>0)', fontsize=14, y=1.02)
        plt.tight_layout()
        plt.savefig('/mnt/staas/CLICHE/00_DATA/EDA/annual_floods/02_boxplots_inundados.png', 
                    dpi=150, bbox_inches='tight')
        plt.show()
        print("Boxplots of flooded pixels saved")
        ...
        ax1.set_ylabel('Mean Flood Frequency (0-1)', color='crimson')
        ax1.tick_params(axis='y', labelcolor='crimson')
        ax1.set_ylim(0, 0.12)  # Appropriate scale for frequency
        ...
        ax1.set_xlabel('Year')
        ax1.set_title('Temporal Evolution of Means')
        ...
        ax2.set_xlabel('Year')
        ax2.set_ylabel('NNumber of Flooded Pixels')
        ...
        ax3.set_title('Percentage of Pixels Never Flooded (0%)')
    ...
    def visualize_spatial_analysis(self):
        """Visualizes spatial analysis and temporal consistency."""
    ...
    def monte_carlo_preparation_optimized(self):
        """
        Prepares specific analysis for Monte Carlo simulation design - OPTIMIZED.
        Uses stratified subsampling (flooded + non-flooded) for speed.
        """
        mc_analysis = {}
        
        for band in self.bands:
            # Strategy: calculate global statistics with all data (fast: mean, std, percentiles)
            # but adjust distributions with a representative sample (max 50k)
            ...
            if len(positive_data) > 100:
                ...
                for dist in distributions:
                    try:
                        if dist == stats.beta:
                            # Beta requires scaling to [0,1] if data is not in that range
                            if band == 'flood_frequency':
    ...
    def visualize_monte_carlo_insights_optimized(self):
        """
        Visualize insights for Monte Carlo - OPTIMIZED VERSION.
        Completely replaces the previous function.
        """
        ...
        # Calculate distance matrix with pdist (C optimized)
        ...
        max_dist = np.percentile(dists, 60)  # Up to the 60th percentile (sufficient for range)
        n_bins = 12
        ...
        plt.savefig('/mnt/staas/CLICHE/00_DATA/EDA/annual_floods/05_monte_carlo_analysis.png', 
                    dpi=150, bbox_inches='tight')
        plt.show()
        print("Optimized Monte Carlo analysis saved")
    ...
    def generate_report(self):
        ...
        for band, analysis in self.mc_analysis.items():
            ...
            print(f"  Interannual variability (mean): {analysis['yearly_variability'].get('mean', 'N/A')}")
            print(f"  Temporal autocorrelation: {analysis['temporal_autocorr']:.3f}")
        
        print("\n" + "=" * 80)
        print("RECOMMENDATIONS FOR MONTE CARLO:")
        print("=" * 80)
        print("""
1. ZERO MODELING: The high probability of pixels with value 0 suggests using 
   zero-inflated models or a two-step approach: (1) probability of flooding, (2) 
   magnitude conditional on flooding.
   
2. TEMPORAL DEPENDENCE: Correlations between years should be modeled with 
   autoregressive processes or temporal copulas.
   
3. SPATIAL STRUCTURE: Use Gaussian random fields (GRF) or sequential simulation
   to maintain the observed spatial autocorrelation.
   
4. DERIVED VARIABLES: flood_duration and max_consecutive_full should be generated 
   conditionally on flood_frequency to maintain physical consistency.
   
5. METHODOLOGICAL UNCERTAINTY: Incorporate the variability in OBSERVED_DAYS 
   as a source of uncertainty in the simulation.
        """)
